...
module.exports = function (userOption) {
     ...
     */
    sdName = function sdName(item) {
        return options.prefix + item.name + options.postfix;
    },
     ...
     */
    validationErrorMessage = function validationErrorMessage(paramName) {
        return "ssr-api-cache ERROR: please set '" + paramName + "' parameter in define staticDataBuilder() place, because this is a require parameter.";
    },
     ...
     */
    fileContextBuilder = function fileContextBuilder(updatedItemName, newValue) {
        var context = "";
        list.forEach(function (item) {
            // built name of item (name with prefix and postfix)
            var name = sdName(item);
     ...
     * @returns {Promise<any>}
     */
    buildJsFile = function buildJsFile(_ref) {
        var madeContext = _ref.madeContext,
            updatedItem = _ref.updatedItem,
            newValue = _ref.newValue;

        // an string contain all static data values. exp: window.__a='a';window.__b='b';
        var fileContext = madeContext || fileContextBuilder(updatedItem.name, newValue);
     ...
     */
    updateItem = function updateItem(updatedItem, newValue) {

        var writeFilePromise = buildJsFile({ updatedItem: updatedItem, newValue: newValue });
     ...
     * @param item
     */
    fetchDataFromApi = function fetchDataFromApi(item) {
        var fetchPromise = new Promise(function (resolve, reject) {
            axios({ url: item.api }).then(function (response) {
                updateItem(item, response.data).then(function () {
     ...
     */
    if (typeof express === 'undefined') throw validationErrorMessage('express'); // express app object
    if (typeof list === 'undefined') throw validationErrorMessage('list'); // list of static data info
    ...
    express.patch(apiRoute + '/:name', function (req, res) {
        ...
        if (Array.isArray(validIP)) {
            var detectList = validIP.filter(function (item) {
                return item === ip;
            });
            //
        ...
        if (targetItem !== null) {
            fetchDataFromApi(targetItem).then(function () {
                status = 200;
